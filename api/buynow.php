...
try {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($stmt->execute()) {
            ...
            $stmt = $db->prepare('insert into sp_transaction (transid,orderlist,amount,shipping,vat,netamount,operation,mil,updated_at) value (?,?,?,?,?,?,?,?,?)');

            if ($stmt->execute([$transid, $product, $amount, $shipping, $vat, $netamount, 'PENDING', $mil, $updated_at])) {
                $object->RespCode = 200;
